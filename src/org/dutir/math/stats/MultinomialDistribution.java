...
public class MultinomialDistribution {
    ...
    public double chiSquared(int[] sampleCounts) {
        checkNumSamples(sampleCounts);
        int totalCount = org.dutir.util.Arrays.sum(sampleCounts);
        double sum = 0.0;
        for (int i = 0; i < sampleCounts.length; ++i)
    ...
    public static double log2MultinomialCoefficient(int[] sampleCounts) {
        ...
        long totalCount = org.dutir.util.Arrays.sum(sampleCounts);
        double coeff = org.dutir.util.Math.log2Factorial(totalCount);
        for (int i = 0; i < sampleCounts.length; ++i)
            coeff -= org.dutir.util.Math.log2Factorial(sampleCounts[i]);
        return coeff;
    }
