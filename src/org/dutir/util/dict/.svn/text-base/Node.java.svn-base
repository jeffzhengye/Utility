 ...
 * @revised by Zheng Ye
 */
public class Node<C> implements Serializable{

    @SuppressWarnings("unchecked") // ok because empty
    ...
    void addEntry(DictionaryEntry<C> entry) {
        ...
        // required for array
        @SuppressWarnings("unchecked")
        DictionaryEntry<C>[] entries = (DictionaryEntry<C>[]) new DictionaryEntry[oldEntries.length+1];
        mEntries = entries;
        mEntries[0] = entry;
        System.arraycopy(oldEntries,0,mEntries,1,oldEntries.length);
    }
    public DictionaryEntry<C>[] getEntries(){
