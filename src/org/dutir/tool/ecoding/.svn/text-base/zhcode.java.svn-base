package org.dutir.tool.ecoding;
...
public class zhcode extends Encoding {
	...
	public zhcode() {
		...
		try {
			...
			// getClass().getResourceAsStream("hcutf8.txt");
	...
	public void hz2gbStringBuffer(StringBuffer hzstring) {
		...
		for (i = 0; i < hzstring.length() ; i++) {
			if (hzstring.charAt(i) == '~') {
				if ((i + 1) >= hzstring.length()) {
					hzstring.deleteCharAt(i);
					break;
				}
				
				
				if(hzstring.charAt(i + 1) == '{') {
					if ((i + 2) >= hzstring.length()) {
						hzstring.deleteCharAt(i);
						break;
					}
...
//				}
				...
				if (autodetect) {
					working_encoding = sinodetector.detectEncoding(tmpfile);
	...
	public static void main(String argc[]) {
		...
		try {
			...
			for (int i = 0; i < 2; i++) {
				...
				// Print Help
				// if (codetype == 'h') {
				// System.out.println("Usage: java zhcode
				// -[gbc867ui2nkhra][gbc867uts2nkhr] in_file out_file");
				// System.out.println(" g = GB2312, b = Big5, c = CNS11643, 8 =
				// UTF-8, 6 = UTF-8 (simp),");
				// System.out.println(" 7 = UTF-8 (trad), u = Unicode, t =
				// Unicode (traditional characters),");
				// System.out.println(" s = Unicode (simplified characters),");
				// System.out.println(" 3 = GB18030,");
				// System.out.println(" i = ISO-2022-CN, 2 = ISO-2022-CN-GB, n =
				// ISO-2022-CN-CNS,");
				// System.out.println(" k = GBK, h = HZ, r = EUC-KR, a =
				// autodetect source encoding");
				// System.exit(0);
				// }
		...
		System.out.println(zhcoder.autodetect);
		int a = 10;
