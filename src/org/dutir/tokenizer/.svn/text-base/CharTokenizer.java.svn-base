package org.dutir.tokenizer;

/**
 * Licensed to the Apache Software Foundation (ASF) under one or more
 * contributor license agreements.  See the NOTICE file distributed with
 * this work for additional information regarding copyright ownership.
 * The ASF licenses this file to You under the Apache License, Version 2.0
 * (the "License"); you may not use this file except in compliance with
 * the License.  You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import java.io.IOException;
import java.io.Reader;

import org.apache.lucene.analysis.Token;
import org.dutir.util.XMLConfigure;

/** An abstract base class for simple, character-oriented tokenizers.*/

/**
 * Why did i code this class? Zheng
 */
public class CharTokenizer {

	private static XMLConfigure conf = XMLConfigure.getConf("lucene-site.xml");
	private int offset = 0, bufferIndex = 0, dataLen = 0;
	private static final int MAX_WORD_LEN = 255;
	private static boolean stemTag = 
		conf.getBoolean("DUTChineseTokenizer.PorterStemmer", false);
	private char[] ioBuffer = new char[0];

	public CharTokenizer(String input){
		this(input, 0);
	}
	
	public CharTokenizer(String input, int offset){
		dataLen = input.length();
		this.ioBuffer = new char[dataLen];
		input.getChars(0, dataLen, ioBuffer, 0);
		bufferIndex = 0;
		this.offset = offset;
	}
	
	public CharTokenizer() {
		// TODO Auto-generated constructor stub
	}

	public void reset(String input) {
		reset(input, 0);
	}
	public void reset(String input,int offset) {
		dataLen = input.length();
		this.ioBuffer = new char[dataLen];
		input.getChars(0, dataLen, ioBuffer, 0);
		bufferIndex = 0;
		this.offset = offset;
	}
	/**
	 * Returns true iff a character should be included in a token. This
	 * tokenizer generates as tokens adjacent sequences of characters which
	 * satisfy this predicate. Characters for which this is false are used to
	 * define token boundaries and are not included in tokens.
	 */
	protected boolean isTokenChar(char c) {
//		byte type = (byte)Character.getType(c);
		switch ((byte)Character.getType(c)) {
		case Character.LOWERCASE_LETTER:
			return true;
		case Character.UPPERCASE_LETTER:
			return true;
		case Character.DECIMAL_DIGIT_NUMBER:
			return true;
		}
		return false;
	}

	/**
	 * Called on each token character to normalize it before it is added to the
	 * token. The default implementation does nothing. Subclasses may use this
	 * to, e.g., lowercase tokens.
	 */
	protected char normalize(char c) {
		return c;
	}

	public final Token next() throws IOException{
		return next(new Token());
	}
	
	public final Token next(Token token) throws IOException {
		if (bufferIndex >= dataLen) {
			return null;
		}
		token.clear();
		int length = 0;
		int start = bufferIndex;
		char[] buffer = token.termBuffer();
		while (true) {
			if (bufferIndex >= dataLen) {
				break;
			}
			final char c = ioBuffer[bufferIndex++];

			if (isTokenChar(c)) { // if it's a token char

				if (length == 0) // start of token
					start = offset + bufferIndex - 1;
				else if (length == buffer.length)
					buffer = token.resizeTermBuffer(1 + length);

				buffer[length++] = normalize(c); // buffer it, normalized

				if (length == MAX_WORD_LEN) // buffer overflow!
					break;

			} else if (length > 0) // at non-Letter w/ chars
				break; // return 'em
		}

		token.setTermLength(length);
		token.setStartOffset(start);
		token.setEndOffset(start + length);
		if(stemTag){
//			token.setTermText(PorterStemmer.stem(token.termText()));
			
		}
//		System.out.println("entoken:" + token);
		return token;
	}

	public static void main(String args[]) throws IOException {
//		System.out.println(PorterStemmer.stem("token"));
		String text = "diseases this12 is a test中国人";
		CharTokenizer tokenizer = new CharTokenizer();
		tokenizer.reset(text,0);
		Token token = new Token();
		
		while ((token = tokenizer.next(token)) != null) {
			System.out.print(token);
		}
	}
	
}
