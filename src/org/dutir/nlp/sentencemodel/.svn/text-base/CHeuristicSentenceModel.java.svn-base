...
/**
 * minor revise of HeuristicSentenceModel: adapting for Chinese Sentence. 
 * Note: 
 * @author zheng
 *
 */
public class CHeuristicSentenceModel extends AbstractSentenceModel {
     ...
     */
    public CHeuristicSentenceModel(Set<String> possibleStops,
...
//        	System.out.println(tokens[i]);
            ...
            if (!CharOper.isAllChinese(tokens[i-1]) && CharOper.isAllEnglishLetter(tokens[i+1]) && whitespaces[i+1].length() == 0) continue;
            ...
            if (CharOper.isAllEnglishLetter(tokens[i-1]) && !possibleStart(tokens,whitespaces,i+1,end)) continue;
